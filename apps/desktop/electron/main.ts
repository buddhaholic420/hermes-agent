...
function createTray() {
  ...
  tray = new Tray(icon)
  tray.setToolTip('Nemobot')
  tray.setContextMenu(
    Menu.buildFromTemplate([
      { label: 'Open Nemobot', click: showMainWindow },
      { type: 'separator' },
      {
        label: 'Quit Nemobot',
        click: () => {
...
function createWindow() {
  ...
  mainWindow.on('close', event => {
    ...
    // On Windows, the title-bar X hides Nemobot into the notification area
    // rather than terminating the desktop process. Explicit tray-menu quits
    // and update handoffs set isQuitting so they still close normally.
